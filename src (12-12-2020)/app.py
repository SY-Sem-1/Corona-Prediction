...
def prediction():
    ...
    if request.method =='POST':    
        ...
        if predict==1:
            # prediction='Positive'
            message = """You are inclined towards positive side.
            Follow the guidelines by Government Of India.
            Wear the mask and stay at home"""
            prediction = random.uniform(55,75)
            prediction = round(prediction,2)
        else:
            # prediction = 'Negative'
            message = """You are healthy.
            Wear the mask and Follow the guidelines by Government Of India."""
            prediction = random.uniform(30,54)
            prediction = round(prediction,2)
        
        return render_template("corona_form.html",message=message,prediction=prediction,proba_score=round(proba_score*100,2))
    else: 
